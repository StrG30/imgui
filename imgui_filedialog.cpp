...
bool ImGui::FileDialog(ImFileDialogInfo* dialogInfo)
{
    if (!dialogInfo->open) return false;

    if (dialogInfo->directoryPath.empty())
    ...
    if (ImGui::Begin(dialogInfo->title.c_str(), &dialogInfo->open))
    {
